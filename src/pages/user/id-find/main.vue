            ...
            <v-col cols="12" lg="7" class="top">
                ...
                <v-form ref="form">
                    <v-row class="mb-8">
                        <v-col cols="12">
                            <v-row>
                                <v-col cols="12" class="row-margin">
                ...
                <v-row style="text-align:right;">
                    ...
                    <v-col cols="12">
                        ...
                        <button class="btn" @click="searchId">아이디 찾기</button>
                    </v-col>
                </v-row>
            </v-col>
...
export default Vue.extend({
    ...
    methods: {
        ...
        async searchId() {
            ...
            if (this.phoneNum == null || this.phoneNuam === '') {
                await meta.alert('Phone Number를 작성해주세요.');
            } else if (validate === false) {
                await meta.alert('유효한 값을 작성해주세요.');
            } else if (await meta.confirm('아이디 찾기를 하시겠습니까?')) {
                const self: any = this;
                const result = (await meta.api.common.user.getUserEmail({ phoneNum: this.phoneNum, firstName: this.firstName, lastName: this.lastName })).data;
                if (!result) {
                    await meta.alert('회원정보가 존재하지 않습니다.');
                }
